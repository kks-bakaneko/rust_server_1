...
pub async fn get_courses_for_tutor_db(pool: &PgPool, tutor_id:i32) -> Result<Vec<Course>,EzyTutorError> {
    ...
    //     _ => Ok(courses),
    // }
    let course_rows: Vec<Course> = sqlx::query_as!(Course, "SELECT * FROM ezy_course_c6 where tutor_id = $1", tutor_id)
        .fetch_all(pool)
        .await?;
...
pub async fn get_course_details_db(pool: &PgPool, tutor_id: i32, course_id: i32) -> Result<Course,EzyTutorError> {
    ...
    // }
    let course_row = sqlx::query_as!(Course, "SELECT * FROM ezy_course_c6 where tutor_id = $1 and course_id = $2", tutor_id, course_id)
        .fetch_optional(pool)
        .await?;
...
pub async fn post_new_course_db(pool: &PgPool, new_course: CreateCourse) -> Result<Course, EzyTutorError> {
    // let course_row = sqlx::query!("insert into ezy_course_c5 (course_id, tutor_id, course_name) values ($1, $2, $3) returning tutor_id, course_id, course_name, posted_time", new_course.course_id, new_course.tutor_id, new_course.course_name.clone())
    //     .fetch_one(pool)
    //     .await?;
    // Ok(Course {
    //     course_id: course_row.course_id,
...
pub async fn delete_course_db(
    ...
    let course_row = sqlx::query!("DELETE FROM ezy_course_c6 where tutor_id = $1 and course_id = $2", tutor_id, course_id)
        .execute(pool)
        .await?;
    Ok(format!("Deleted {} record", course_id))
}
...
pub async fn update_course_details_db(
    ...
    let current_course_row = sqlx::query_as!(
        ...
        .fetch_one(pool)
        .await
        .map_err(|_err| EzyTutorError::NotFound("Course id is not found".into()))?;
    ...
    let course_row = sqlx::query_as!(
        Course,
        "UPDATE ezy_course_c6 set course_name = $1, course_description = $2, course_format = $3, course_structure = $4, course_duration = $5, course_price = $6, course_language = $7, course_level = $8 where tutor_id = $9 and course_id = $10 returning tutor_id, course_id, course_name, course_description, course_duration, course_level, course_format, course_language, course_structure, course_price, posted_time",
        name,
        description,
        format,
        structure,
